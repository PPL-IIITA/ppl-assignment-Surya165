...
public class Problem7 {
	...
	public static void main(String[] args) throws FileNotFoundException, UnsupportedEncodingException {
		...
		/*This part of the code sorts the girls array by maintanance*/
		girl tempGirl = new girl();
		for ( i = 0; i < numberOfGirls; i ++ )
		{
			for (int j  = i + 1; j < numberOfGirls; j ++ )
			{
				if(g[i].Maint > g[j].Maint )
				{
					tempGirl = g[i];
					g[i]= g[j];
					g[j] = tempGirl;	
		...
		boy tempBoy = new boy();
		/*This part of the code sorts the boys list according to attraction*/
		for ( i = 0; i < numberOfBoys; i ++ )
		{
			for ( int j = i + 1; j < numberOfBoys; j ++ )
			{
				if ( b[i].Attr > b[j].Attr)
				{
					tempBoy = b[i];
					b[i] = b[j];
					b[j] = tempBoy;
		...
		//For further use, since the boy chooses the most attractive girl Available, it is best to sort the girls according to attraction
		for ( i = 0; i < numberOfGirls; i ++ )
		{
			for ( int j = i + 1; j < numberOfGirls ; j ++ )	
			{
				if( g[i].Attr < g[j].Attr )
				{
					tempGirl = g[i];
					g[i] = g[j];
					g[j] = tempGirl;
